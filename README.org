...
Most existing "download a free template" systems only give users a single document. But, often, multiple documents are needed to consummate a given business operation. We produce a /Document Set/, not a single document.
...
Most existing services limit document creation to the existing priesthood. While confidence in the system is important, we want to be open, allowing anyone to contribute, fork, and republish templates. We think we can develop confidence in other ways more consonant with the age of the platform. In other words, reputation systems with transparency.
