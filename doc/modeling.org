...
#+BEGIN_SRC yaml
my fundraise is an InvestmentFinancingWorkflow.

my company is a Company with:
  id: 2016000001A
  name: My Example Inc.

my safe is a SafeInstrument.

the safe has parties:
  investors:
    - name: Alice, id: S1111111A, purchaseAmount: 20000
    - name: Bob,   id: S2222222B, purchaseAmount: 10000
    - name: Carol (defined elsewhere)
  company: (defined elsewhere)
...
the safe also has:
  agreementDate: 2016-02-30

a SafeInstrument is a Security.
...
a SafeInstrument generally has:
  explode: one doc per investor
  date(): investor.date || today()
...
a SafeInstrument generally has:
  ...
  intro: {
    "this document" certifies {
      consideration({from: inv, to: company}),
      hereby( issues( rightTo( shares( subjectTo( terms ) ) ) ) ) )
  }
  topDefinitions: company.terms.FILTER(discount,valuation,maturity)
  events:
    'equity financing': {
      upon(parties.company.event.financing.equityIssue ('equityIssue') and
		   equityIssue.has(preMoneyValuation) and
		   equityIssue.stage=='primary negotiation complete' and
           equityIssue.closingDate < .terminationDate) {
		...
		company.must(automatically): {
          given conversionPrice = equityIssue.pricePerShare
          given numShares = investor.purchaseAmount / conversionPrice

		  safe.parties.investors(each an 'investor').all(
			company.addObligation(upon(equityIssue.stage=='completion' and
									   investor.EFdocs.all(_.executed(_) and _.received_by(company))
									  ) {
			  .issue(equityIssue.shareClass, quantity:numShares).to(investor) ('ShareIssue')
			})
          }
		}
        ...
        investor.must(.EFdocs.all(_.parties.any(|p| EFpurchasers.any(_ == p)))
                      and
                      (.EFdocs.all(not _.has(dragAlongRights) or
								   _.has(dragAlongRights) and
								   _.dragAlongRights.applyTo(investor) and
								   _.has(exceptions.dragAlongRights or dragAlongRights.exceptions) ('dragE') and
