...
#+BEGIN_SRC text
  ...
  rule toilets governs hotel {
      ...
      hotel.must ("toilets") {
        if (numMaleToilets > 0) { numFemaleToilets >= numMaleToilets }
        else                    { numUnisexToilets  > 0 }
      }
  }
#+END_SRC
...
#+BEGIN_SRC text
  ...
  rule babyrooms governs hotel = mutatis(hotel.rule(toilets))
#+END_SRC text
