...
We have three equations with three unknowns: ppsNewround, conversionShares, ppsDiscount

ppsDiscount = (1-discount) * ppsNewround;
ppsNewround = newMoney / (existingShares + conversionShares);
conversionShares = conversionAmount / ppsDiscount;

discount, newMoney, existingShares, and conversionAmount are known.

These are nonlinear equations for which a specialized solver is necessary.
