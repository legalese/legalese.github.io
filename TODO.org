...
* TODO poll the e-signature backend to find out when a document is signed. update the spreadsheet.
...
* TODO integration with third party project management software like task management and calendaring
because the signing of a contract creates certain obligations which if unmet constitute breach

so a company needs to take those obligations seriously.

* DONE write to the Readme the list of To: and CC: for when the user is submitting to EchoSign manually.
* TODO import each document in the Y Combinator Series AA series
particularly the CoI amendments to the articles of association
for taembe
...
* DONE fix the document numbering -- when there are more than 9 documents, the 1-9 don't get 0 prefixed, but 10 does, so you get 010 lol
...
* TODO [#C] add qryo-style QR stamping on each page
...
* DONE latin phrase for holy water
...
* DONE make Legalese a library
that people can just import, similar to the OAuth library.

* TODO [#A] make Legalese an add-on
that doesn't import a library.

* TODO [#A] obtain a CUSTOMER api key for integration against echosign; get blessing.
* for jfdi.2014
...
* DONE clean up the overlap between the new share class creation variables and the new share issue variables. 
...
* TODO add gender pronoun for parties
* DONE for xml2pdf-lib.jsx, any paragraph that ends in ":" or ":--+" should have a "keep with next" turned on.
